...
/// Multi-channel-output condition_dsp matches NAMCore: the nested condition_dsp emits
/// N output channels (N == the parent arrays' `condition_size`), which become the
/// N-wide planar conditioning fed to every array (the raw input still drives the first
/// array's layer input). Uses the turnkey `wavenet_condition_dsp.nam` example
/// (`condition_size == 3`, condition_dsp out 3). The input/expected fixtures are
/// gitignored (oracle-generated by `make_condition_dsp_multi.py`); skips if absent.
#[test]
fn multi_channel_condition_dsp_matches_namcore_oracle() {
    if !fixture("input_condition_dsp_multi.json").exists()
        || !fixture("expected_condition_dsp_multi.json").exists()
    {
        eprintln!("skip multi_channel_condition_dsp: fixture missing");
        return;
    }
    let json = std::fs::read_to_string(fixture("wavenet_condition_dsp.nam"))
        .unwrap_or_else(|e| panic!("missing fixture wavenet_condition_dsp.nam: {e}"));
    let model = NamModel::from_json_str(&json).expect("parse multi-channel condition_dsp model");
    let mut net = Model::from_nam(&model).expect("multi-channel condition_dsp model builds");
    let input = load_samples("input_condition_dsp_multi.json");
    let expected = load_samples("expected_condition_dsp_multi.json");
    assert_eq!(input.len(), expected.len());
    let rf = net.receptive_field();
    let mut signal = input.clone();
    net.process_buffer(&mut signal);
    let max_err = signal[rf..]
        ...
        .map(|(g, w)| (g - w).abs())
        .fold(0.0_f32, f32::max);
    eprintln!("multi_channel_condition_dsp max steady-state error: {max_err:.3e}");
    assert!(
        max_err <= TOLERANCE,
        "multi-channel condition_dsp max steady-state error {max_err} > {TOLERANCE}"
    );
}
